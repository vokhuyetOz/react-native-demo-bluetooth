...
const App = () => {
  ...
  const readFirstConnectdPeripherals=()=>{
    ...
    const data = await  BleManager.read(service.id,service.characteristics[0].service,service.characteristics[0].characteristic);
  ...
  useEffect(() => {
    ...
    const checkPermission = async ()=>{
      const permission = Platform.select({android: PERMISSIONS.ANDROID.ACCESS_FINE_LOCATION,
      ...
      <SafeAreaView>
        <ScrollView
          ...
          <View style={styles.body}>
            ...
            <View style={{margin: 10}}>
              <Button title="Read data from connected peripherals" onPress={() => readFirstConnectdPeripherals() } />
            </View>
            {(list.length == 0) &&
